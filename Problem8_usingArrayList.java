...
public class Problem8_usingArrayList {
    public static void main(String[] args) {
        List<Integer> nums = new ArrayList<>(Arrays.asList(0, 1, 2, 2, 3, 0, 4, 2));
        int val = 2, k = 0;

        ListIterator<Integer> ltr = nums.listIterator();
        while (ltr.hasNext()) {
            int next = (int) ltr.next();
            if (val == next)
                ltr.remove();
            else
                k++;
